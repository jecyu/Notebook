...
观察上面列出 i 的取值发现，是一个等比数列，要知道循环了多少次，求出 x 的值即可。由 2^x = n 得到，x = log2 n。

如果把上面的 i*2 = 2 改为 i*=3，那么这段代码的时间复杂度就是 log3n。

根据换底公式：

logca * logab = logcb

因此 log3n = log32 * log2n，而 log32 是一个常量，得到 O(log3(n)) = O(log2(n))。所以，在对数时间复杂度的表示中，我们忽略对数的“底”，我不管你底数是多少，统一计作 O(log(n))。
