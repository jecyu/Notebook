...
。无论是故事剧情、人物，还是制造一个工业产品，所谓原型就是最初的样例、模型或者是为了测试概念而构建的产品。
...
[一个机器人或虐杀原型，从纸面到真正可以动的模型]()

### 原型经历的阶段

纸面原型 -> 

从原型到成品，也不是一次到位，其中也经历了很多的版本迭代，跟我们编程一样的道理。
...
2K NBA online，已经具备基本的篮球能力的原型，开始只有基本的人物裸体，然后进行各种各种的更换衣服，脸型。

除了这种模型外，还有数据模型等。

### 构造器与原型的区别

在编程中，除了 原型（`prototype`） 这个词外，构造器（`contructor`）也是我们接触比较多的术语了。
