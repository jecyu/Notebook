...
相对 Webpack，Rollup 更轻量，只处理 JavaScript。
...
```js
let builds = require('./config').getAllBuilds() // 拿到构建的所有配置
...
> Vue 运行时 + 编译器版本使用参考：https://vuejs.org/v2/guide/installation.html#Runtime-Compiler-vs-Runtime-only

```js
// vue.config.js vuecli 4.x的配置
module.exports = {
  configureWebpack: {
    resolve: {
      alias: {
