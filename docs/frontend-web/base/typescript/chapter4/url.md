...
这种是通常约定的格式，如果后端报错，说明后端没能对这种格式做很好的处理支持。
...
export function isDate (val: any): val is Date { // 返回类型：使用类型谓词而不是 boolean 类型，这样在使用 isDate 的上下文中，才有类型提示，比如方法，这里也可以看出 any 的用处是有的，后面可以用 unknown 类型替换掉
