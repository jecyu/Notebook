...
>（PS：直接把 token 通过 set-cookie 种到 cookie 上，是不安全的，一样容易受到 csrf 的攻击，可以把它放到 localStorage 更安全，并且让服务器对 token 做超时处理。当然如果网站有 XSS 漏洞的话，一样可以读取 localStorage 的信息，将JWT储存在Local Storage中然后通过请求中的Authorization Header发送，注意 Header 不是由浏览器自动添加。） 
...
同域名的判断主要利用了一个技巧，创建一个 a 标签的 DOM，然后设置 `href` 属性为我们传入的 `url`，然后可以获取该 DOM 的 `protocol`、`host`。当前页面的 `url` 和请求的 `url` 都通过这种方式获取，然后对比它们的 `protocol` 和 `host` 是否相同即可。（或者直接使用 new URL(url) 对象来获取，一般来说公网的页面，不会直接提供端口访问的，如果是内网的话，还要加上端口的处理。）
