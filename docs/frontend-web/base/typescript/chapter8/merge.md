...
定义了默认配置后，我们发送每个请求的时候需要把自定义配置和默认配置做合并，它并不是简单的 2 个普通对象的合并，对于不同的字段合并，会有不同的合并策略。（比如 Vue mixin 也有不同的合并策略）举个例子：
...
有同学可能会问，为什么不直接用 lodash 的深拷贝，因为我们这里考虑的因素没有那么多，可以自己实现。
```typescript
export function deepMerge(...objs: any[]): any {  // 并不是所有的类型都能提前，所以 ts 允许使用 any 声明
