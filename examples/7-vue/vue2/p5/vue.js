...
(function(global, factory) {
  ...
  function createPatchFunction(backend) {
    ...
    // 新子节点 和 旧子节点，循环遍历逐个比较
    // 1. 使用 while 2. 新旧节点数组都配置首尾两个索引
    function updateChildren(
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        if (isUndef(oldStartVnode)) {
          ...
          canMove && // 以 新子节点的位置 来移动的，旧头 在新子节点的 末尾
            nodeOps.insertBefore(
              parentElm,
              oldStartVnode.elm,
              nodeOps.nextSibling(oldEndVnode.elm) // 所以把 oldStartVnode 的 dom 放到 oldEndVnode 的后面
            );
          oldStartVnode = oldCh[++oldStartIdx];
          newEndVnode = newCh[--newEndIdx];
          ...
          // 单个遍历查找，前面的逻辑都不满足的情况下，拿新子节点的子项，直接去旧子节点数组中遍历，找一样的节点出来
          if (isUndef(oldKeyToIdx)) {
            // 生成旧子节点数组以 vnode.key 为 key 的 map 表
            oldKeyToIdx = createKeyToOldIdx(oldCh, oldStartIdx, oldEndIdx);
          }
          // 拿到新子节点数组中 一个子项，判断它的key是否在上面的map 中，属性名是 vnode.key，属性值是 该 vnode 在children 的位置
          idxInOld = isDef(newStartVnode.key)
            ? oldKeyToIdx[newStartVnode.key]
            : findIdxInOld(newStartVnode, oldCh, oldStartIdx, oldEndIdx);
          if (isUndef(idxInOld)) {
            // New element  不存在，则新建DOM
            createElm(
              newStartVnode,
              insertedVnodeQueue,
              parentElm,
            ...
            if (sameVnode(vnodeToMove, newStartVnode)) {
              ...
              canMove &&
                nodeOps.insertBefore(
                  /// 如果相同，直接移动到 oldStartVnode.elm 前面
                  parentElm,
                  vnodeToMove.elm,
                  oldStartVnode.elm
                );
      ...
      if (oldStartIdx > oldEndIdx) {
        // 旧子节点遍历完毕，新子节点可能有剩
        // 所以要对剩余的新子节点处理
        refElm = isUndef(newCh[newEndIdx + 1]) // 如果 newEndIdx 没有移动过，一直是最后一位，那么就不存在 newCh[newEndIdx + 1]
          ? null
          : newCh[newEndIdx + 1].elm; // 根据 refElm 来判断插入哪里
        addVnodes(
          parentElm,
          refElm,
          newCh,
    ...
    function patchVnode(
      ...
      if (isUndef(vnode.text)) {
        ...
        if (isDef(oldCh) && isDef(ch)) {
          ...
          nodeOps.setTextContent(elm, ""); // 文本节点为 ''，直接把文本 DOM 赋值为空
        }
      } else if (oldVnode.text !== vnode.text) {
        // 文本节点，则更新文本，文本节点不存在子节点
    ...
    return function patch(oldVnode, vnode, hydrating, removeOnly) {
      ...
      if (isUndef(oldVnode)) {
        ...
        if (!isRealElement && sameVnode(oldVnode, vnode)) {
          // patch existing root node 比较存在的根节点
          patchVnode(
            oldVnode,
            vnode,
            insertedVnodeQueue,
          ...
          // replacing existing element 替换存在的元素
