      ...
      <div class="table-container">
        ...
        >
          ...
          <template slot="checkResultStatus" slot-scope="{ row }">
            <Tooltip
              transfer
              v-if="row.checkResultStatus === EnumCheckResutlStatus.checkError"
              placement="top-start"
              theme="light"
              max-width="200"
              :content="checkResultStatusMap(row.checkResultStatus)"
            >
              <EdSvgIcon
                iconClass="error"
            ...
            <Tooltip
              transfer
              v-else-if="
                row.checkResultStatus === EnumCheckResutlStatus.checkSuccess
              "
              placement="top-start"
              theme="light"
              max-width="200"
            ...
            <Tooltip
              transfer
              v-else-if="
                row.checkResultStatus === EnumCheckResutlStatus.checkNotice
              "
              placement="top-start"
              theme="light"
              max-width="200"
            ...
            <span v-else>{{
              checkResultStatusMap(row.checkResultStatus)
            }}</span>
          </template>
...
enum EnumRepeatedStatus {
  notRepeated, // 0：新增(不重复)
  repeated // 1:覆盖(重复)
}

// enum EnumBuildStatus { // 不能直接在 template 引用
//   error,
//   success,
//   building,
//   waiting
...
interface RowData {
  rank: number;
  name: string;
  checkResultStatus: number | string;
...
export default class DASC extends mixins(UploadFileMixin) {
  ...
  private async InitGetUploadArchive() {
    const { EnumCheckResutlStatus, EnumBuildStatus } = this;
    // 组织参数
    const param = {
    ...
    this.showData = (result as any).map((item: any, index: number) => {
      ...
      let checkResultStatus = EnumCheckResutlStatus.waitCheck; // 默认等待中
      let buidStatus = EnumBuildStatus.waiting;
  ...
  // private async UpDateShowData(refRequest: Array<any>) {
  //   // 给当前的 ShowData 添加值，为了马上响应上传状态
  //   // 后续轮询的时候，再把它替换掉，TODO处理 // 有问题，因为新添加的
  //   refRequest.forEach((item: any) => {
  //     // const { fileData } = item;
  //     item.checkResultStatus = this.EnumCheckResutlStatus.waitCheck;
  //     const obj = Object.assign(item, {
  //       // name: fileData.name
  //     });
  //     this.showData.push(obj); // push 才能响应式
  //   });
  ...
  private HandleCancelUpload(row: any) {
    ...
    row.checkResultStatus = this.EnumCheckResutlStatus.checkError;
  }
  /**
   * 删除当前的档案记录
   */
